...
function isInboxTriageWork(text: string) {
  ...
  if (!/\b(triage|catch up|catch|decisions?|records?|requests?|organize|context|compare|options?|plan|next steps?|actionable|sources?|assumptions?|gaps?|draft|artifact|version|configuration|calculation|review|approve|approved|approval|proposed work|quality|risk|risks?|criteria|defects?|omissions?|uncertainty|corrections?|coordinate|carry out|requested action|completed correctly|execute|execution|scope|targets?|recipients?|permissions?|timing|confirm|confirmation|rollback|external|irreversible|high impact|track|progress|exceptions?|status|signals?|thresholds?|trends?|follow-?ups?|stale|noise|alerts?|remediation|close|closing|closeout|capture lessons?|lessons?|lifecycle|completion|dependencies|retention|access rules?|evidence|preserv(?:e|es|ed|ing)|archives?|archiving|restores?|cancels?|retires?|disposes?|disposal|hold|recoverable|recoverability)\b/.test(text)) return false;

  return /\b(catch up|catch|find|key|organize|compare|plan|next steps?|actionable|looking across|show.*sources?|assumptions?|gaps?|draft|artifact|version|configuration|calculation|review|approve|approved|approval|proposed work|quality|risk|risks?|criteria|defects?|omissions?|uncertainty|corrections?|coordinate|carry out|requested action|completed correctly|execute|execution|scope|targets?|recipients?|permissions?|timing|confirm|confirmation|rollback|external|irreversible|high impact|track|progress|exceptions?|status|signals?|thresholds?|trends?|follow-?ups?|stale|noise|alerts?|remediation|close|closing|closeout|capture lessons?|lessons?|lifecycle|completion|dependencies|retention|access rules?|evidence|preserv(?:e|es|ed|ing)|archives?|archiving|restores?|cancels?|retires?|disposes?|disposal|hold|recoverable|recoverability)\b/.test(text);
}
