...
function isInboxTriageWork(text: string) {
  ...
  if (!/\b(triage|catch up|catch|decisions?|records?|requests?|organize|context|compare|options?|plan|next steps?|actionable|sources?|assumptions?|gaps?|draft|artifact|version|configuration|calculation|review|approve|approval|proposed work|quality|risk|criteria|defects?|omissions?|uncertainty|corrections?)\b/.test(text)) return false;

  return /\b(catch up|catch|find|key|organize|compare|plan|next steps?|actionable|looking across|show.*sources?|assumptions?|gaps?|draft|artifact|version|configuration|calculation|review|approve|approval|proposed work|quality|risk|criteria|defects?|omissions?|uncertainty|corrections?)\b/.test(text);
}
